...
AExperimentalCubeTwo::AExperimentalCubeTwo() 
{
	...
	CubeMesh->SetStaticMesh(ConstructorHelpers::FObjectFinder<UStaticMesh>(TEXT("/Game/StarterContent/Shapes/Shape_Cube.Shape_Cube")).Object);
	...
	else
	{
		if (CrtTime > 0.5f)
		{
			...
			if (CrtTime > 1.0f && !bHasAppliedForce)
			{
				if (bIsImpulseBased)
				{
					// The constant 0.001 comes from the frame duration of Gazebo, so that the actual forces applies match in both Engines.
